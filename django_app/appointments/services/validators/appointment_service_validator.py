...
class AppointmentServiceValidator:
    ...
    def validate_appointment_data(self, appointment_data, is_update=False):
        ...
        if is_update:
            ...
            self.validate_updated_at(appointment_data.get('updated_at', None))
        else:
            self.validate_created_at(appointment_data.get('created_at', None))
            self.validate_updated_at(appointment_data.get('updated_at', None))
    ...
    def validate_starts_at_ends_at(self, starts_at, ends_at):
        ...
        if starts_at or ends_at is None:
    ...
    def validate_status(self, status):
        ...
        if status not in self.ALLOWED_APPOINTMENT_STATUSES:
            raise ValidationError(f"status must be one of the following: {self.ALLOWED_APPOINTMENT_STATUSES}")
    ...
    def validate_confirmation_number(self, confirmation_number, appointment_id):
        ...
        if confirmation_number:
            ...
            appointment = self.appointment_repository.get_appointment_by_confirmation_number(confirmation_number)
            if len(appointment) > 1:
                raise ValidationError(f"Multiple appointments ({appointment}) with same confirmation number. Confirmation number must be unique")
            if appointment and appointment.id != appointment_id:
                raise ValidationError(f"Appointment with confirmation number already exists ({appointment}). Confirmation number must be unique")
